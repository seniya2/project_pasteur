(function() {
  'use strict';
  	...
  	boardPointUi1Controller.$inject = ['config', '$scope', '$resource', '$http', '$location', '$filter' ];
    function boardPointUi1Controller (config, $scope, $resource, $http, $location, $filter) {
    	Messenger.options = {
		...
		$scope.template_base = "/appReference/modules/board-point-ui1/";
		$scope.template = $scope.template_base + "board-point-ui1-list.html";
...
*/
		...
		$scope.listAction = function(pageNumber, size) {
			...
			$http(
					{
						method : 'GET',
						url : $scope.baseRestUrl + 'point?page=' + pageNumber + '&size=' + size + '&sort='+sort
					}).success(function(data) {
				$scope.pointList = data._embedded.point;
				$scope.page = data.page;
				console.log($scope.pointList);
		...
		$scope.viewAction = function(point) {
			...
			$scope.template = $scope.template_base + "board-point-ui1-view.html";
		}
		...
		$scope.editAction = function(point) {
			...
			$scope.template = $scope.template_base + "board-point-ui1-edit.html";
		}
  ...
  angular.module('singApp.board-point-ui1').controller('boardPointUi1Controller', boardPointUi1Controller);
