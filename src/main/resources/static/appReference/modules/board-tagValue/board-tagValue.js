(function() {
  'use strict';
  	...
  	boardTagValueController.$inject = ['config','$scope', '$resource', '$http', '$location', '$filter' ];
    function boardTagValueController (config, $scope, $resource, $http, $location, $filter) {
    	Messenger.options = {
		...
		$scope.template_base = "appReference/modules/board-tagValue/";
		$scope.template = $scope.template_base + "board-tagValue-list.html";
		...
		$scope.searchUrlBase = $scope.baseRestUrl + "tagValue/search/";
		...
		$scope.prepareAction = function() {
			...
			$http(
					{
						method : 'GET',
						url : $scope.baseRestUrl + 'tagValue?tag=' + s_page	+ '&size=' + s_size + '&sort=' + s_sort
					}).success(function(data) {
				$scope.dataList = data._embedded.tagValue;
		...
		$scope.listAction = function(pageNumber, size) {
			...
			$http(
					{
						method : 'GET',
						url : $scope.baseRestUrl + 'tagValue?page=' + pageNumber + '&size=' + size + '&sort='+sort
					}).success(function(data) {
				$scope.dataList = data._embedded.tagValue;
				$scope.page = data.page;
				console.log($scope.dataList);
		...
		$scope.createAction = function(point) {
			...
			$http({
				method : 'POST',
				url : $scope.baseRestUrl + 'tagValue',
				headers : {
		...
		$scope.updateAction = function(point) {
			...
			$http({
				method : 'PUT',
				url : $scope.baseRestUrl + 'tagValue/' + point.no,
				headers : {
					'Content-Type' : 'application/json; charset=UTF-8'
				},
				data : point
		...
		$scope.deleteAction = function(id) {
			$http({
				method : 'DELETE',
				url : $scope.baseRestUrl + 'tagValue/' + id
			}).success(function(data) {
				//console.log(data);
				$scope.template = $scope.template_base + "board-tagValue-list.html";
				$scope.listAction($scope.page.number, $scope.page.size);
		...
		$scope.viewAction = function(point) {
			...
			$scope.template = $scope.template_base + "board-tagValue-view.html";
		}
		...
		$scope.editAction = function(point) {
			...
			$scope.template = $scope.template_base + "board-tagValue-edit.html";
		}
  ...
  angular.module('singApp.board-tagValue').controller('boardTagValueController', boardTagValueController);
